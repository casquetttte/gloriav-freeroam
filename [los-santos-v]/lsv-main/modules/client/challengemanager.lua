...
function ChallengeManager.Request(opponent, challengeName, acceptEventName, ...)
	...
	while requestTimer and requestTimer:Elapsed() < Settings.challenge.requestTimeout do
		...
		Gui.DisplayHelpText('Appuyez sur ~INPUT_VEH_HEADLIGHT~ pour accepter '..challengeName..'.')
...
function ChallengeManager.Finish(winner, looser, challengeName)
	if winner and winner ~= Player.ServerId() and looser ~= Player.ServerId() then
		Gui.DisplayNotification(Gui.GetPlayerName(winner)..' a vaincu '..Gui.GetPlayerName(looser)..' dans '..challengeName..'.')
		return false
	end
	...
	if Player.IsActive() then
		...
		scaleform:Call('SHOW_SHARD_MIDSIZED_MESSAGE', isWinner and 'VAINQUEUR' or 'PERDANT', '')
		scaleform:RenderFullscreenTimed(10000)
		scaleform:Delete()
	else
		Gui.DisplayPersonalNotification(isWinner and 'Vous avez gagné '..challengeName..'.' or 'Vous avez perdu '..challengeName..'.')
	end
