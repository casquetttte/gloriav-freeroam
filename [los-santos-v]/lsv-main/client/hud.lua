		...
		if eventFinishedTime then
			if Player.IsInFreeroam() then
				...
				if timePassed > 0 then
					Gui.DrawTimerBar('PROCHAIN ÉVÈNEMENT DANS', timePassed, 1)
				end
			end
		end
	end
...
AddEventHandler('lsv:init', function()
	...
	local tips = {
		'Maintenez ~INPUT_MULTIPLAYER_INFO~ pour voir le scoreboard.',
		'Effectuer des missions et éliminer des joueurs ennemis vous donnera de l\'argent et de l\'expérience.',
		'Obtenez une récompense supplémentaire pour avoir tué des joueurs qui effectuent une mission.',
		'Les défis vous permettent de rivaliser avec d\'autres joueurs pour prouver vos compétences.',
		'Appuyez sur ~INPUT_INTERACTION_MENU~ pour ouvrir le Menu Interaction.',
		'Visitez ~BLIP_GUN_SHOP~ pour acheter des Munitions Spéciales.',
		'Appuyez sur K pour ouvrir le Menu de véhicule personnel.',
		'Appuyez sur ~INPUT_ENTER_CHEAT_CODE~ pour agrandir le radar.',
		'Appuyezs sur ~INPUT_DUCK~ pour entrer en mode furtif et se cacher du radar.',
		'Visitez ~BLIP_CLOTHES_STORE~ pour changer l\'apparence de votre personnage.',
		'Utilisez le Menu Interaction pour gérer votre Crew.',
		'Utilisez l\'option Signaler le joueur du menu Interaction pour améliorer votre expérience de jeu globale.',
		'Utilisez l\'option Prestige du menu Interaction pour réinitialiser votre progression et obtenir un badge Prestige.',
	}
...
AddEventHandler('lsv:playerConnected', function(player)
	...
	if PlayerId() ~= playerId and NetworkIsPlayerActive(playerId) then
		Gui.DisplayNotification(Gui.GetPlayerName(player)..' connecté.')
...
AddEventHandler('lsv:onPlayerDied', function(player, suicide)
	if NetworkIsPlayerActive(GetPlayerFromServerId(player)) then
		if suicide then
			Gui.DisplayNotification(Gui.GetPlayerName(player)..' s\'est suicidé.')
		else
...
AddEventHandler('lsv:onPlayerKilled', function(player, killer, message)
	...
	elseif killstreak == 7 then killstreakMessage = 'LEGENDARY KILL'
...
AddEventHandler('lsv:init', function()
	while true do
		...
		if Player.PatreonTier == 0 then
			...
			Gui.SetTextParams(4, { r = 255, g = 255, b = 255, a = 255 }, 0.45, true, false, true)
			Gui.DrawText('~c~Appuyez sur ~w~K~c~ pour ouvrir le Menu de véhicule personnel', { x =SafeZone.Left() + 0.785, y = SafeZone.Top() + 0.025 })
		end
		...
		elseif IsPlayerDead(PlayerId()) and DeathTimer then
			...
			Gui.DrawProgressBar('RÉAPPARITION', GetTimeDifference(GetGameTimer(), DeathTimer) / TimeToRespawn, 0, Color.GetHudFromBlipColor(Color.BLIP_RED))
		else
...
AddEventHandler('lsv:init', function()
	...
	respawnFasterScaleform:Call('SET_DATA_SLOT', 0, '~INPUT_ATTACK~', 'Réapparaître plus vite')
		...
		if IsPlayerDead(PlayerId()) then
			...
			if killer > 0 and killer ~= PlayerPedId() then
				...
				if weaponHash and weaponHash ~= 0 then
					...
					if weaponName then
						deathDetails = 'Tué avec '..weaponName..'\n'..deathDetails
					end
				end
			end

			scaleform:Call('SHOW_SHARD_WASTED_MP_MESSAGE', '~r~MORT', deathDetails)
...
AddEventHandler('lsv:setupHud', function(hud)
	...
	if hud.discordUrl ~= '' then
		...
		while true do
			...
			FlashMinimapDisplay()
			Gui.DisplayPersonalNotification('Rejoignez notre Discord pour lire les derniers changements et rester avec la communauté.', 'CHAR_MP_STRIPCLUB_PR', 'Invitation Discord', discordUrl, 2)

			Citizen.Wait(Settings.discordNotificationTimeout)
			PlaySoundFrontend(-1, 'EVENT_START_TEXT', 'GTAO_FM_EVENTS_SOUNDSET', true)
			FlashMinimapDisplay()
			Gui.DisplayPersonalNotification('Gagnez plus de récompenses et obtenez des bonus exclusifs dans le jeu.\nLisez plus de détails dans notre Discord.', 'CHAR_MP_STRIPCLUB_PR', 'Devenez VIP', discordUrl, 2)
		end
	end
end)
