...
AddEventHandler('lsv:startHuntTheBeast', function(data, passedTime)
	...
	Citizen.CreateThread(function()
		if Player.IsInFreeroam() and not passedTime then
			Gui.StartEvent('Hunt The Beast', World.BeastPlayer == Player.ServerId() and 'Vous êtes la bête. Visitez '..beastData.totalLandmarks..' repères dans le temps imparti.'
				or 'Traquez et tuez la bête '..beastData.livesLeft..' fois.')
		end
		...
		while true do
			...
			if Player.IsInFreeroam() then
				Gui.DisplayObjectiveText(World.BeastPlayer == Player.ServerId() and 'Aller aux ~y~repères~w~.' or 'Tuer la ~r~bête~w~.')

				Gui.DrawTimerBar('ÉVÈNEMENT TERMINÉ', math.max(0, Settings.huntTheBeast.duration - GetGameTimer() + beastData.startTime), 1)
				Gui.DrawBar('REPÈRES RESTANTS', beastData.totalLandmarks - beastData.landmarksPicked, 2)
				Gui.DrawBar('VIES DE LA BÊTE', beastData.livesLeft, 3)
			end
		end
	end)
...
AddEventHandler('lsv:finishHuntTheBeast', function(winner)
	...
	local messageText = winner and Gui.GetPlayerName(winner)..' a gagné.' or 'La bête a perdue.'
	if Player.IsInFreeroam() then
		...
		scaleform:Call('SHOW_SHARD_MIDSIZED_MESSAGE', isPlayerWinner and 'VANQUEUR' or 'ÉVÉNEMENT TERMINÉ', messageText, 21)
...
AddEventHandler('lsv:huntTheBeastLandmarkCollected', function(index)
	...
	if World.BeastPlayer == Player.ServerId() then
		...
		Gui.DisplayNotification('La bête a visitée un repère.')
	end
end)
